...
const Quotify = ({ userIdProp, onLogout = () => {} }) => {
  ...
  const fetchUserInfo = async () => {
    try {
      const res = await fetch(
        `${BASE_URL}/api/user/getuserdetails?userId=${userId}`
      );
  ...
  const fetchFavorites = async () => {
    try {
      const res = await fetch(
        `${BASE_URL}/api/quote/getfavorites?userId=${userId}`
      );
  ...
  const removeFavorite = async (favoriteId) => {
    try {
      const res = await fetch(
        `${BASE_URL}/api/quote/favorite/${favoriteId}?userId=${userId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const generateQuotes = async () => {
    if (!topic.trim() || !userId || !quoteCount) {
    ...
    try {
      ...
      if (data && data.quotes && Array.isArray(data.quotes)) {
        ...
        if (
          data.quotes.some((q) => q.includes("```") || q === "[" || q === "]")
        ) {
          // Extract the actual quote strings from the array
          const quoteStrings = data.quotes.filter(
            (q) =>
              q !== "```json" &&
              q !== "```" &&
              q !== "[" &&
        ...
        // Case 2: If we have an array but one item looks like a JSON string array ["quote1", "quote2"]
          ...
          typeof data.quotes[0] === "string" &&
          data.quotes[0].startsWith("[") &&
          data.quotes[0].endsWith("]")
        ) {
          try {
      ...
      <main className='max-w-6xl mx-auto space-y-6'>
        <section className='bg-white rounded-xl shadow-lg p-6 text-center'>
          <h1 className='text-3xl md:text-4xl font-bold mb-4'>
            AI-Powered Quotes Generator
          </h1>
          <p className='text-gray-600 mb-6 max-w-2xl mx-auto'>
            Create meaningful and inspiring quotes...
          </p>
            ...
            <div className='relative overflow-hidden rounded-lg'>
              ...
              >
                {quotes.map((quote, idx) => (
                  <div
                    key={idx}
                    className='min-w-full p-6 bg-gray-50 rounded-lg'
                  >
                    <div className='relative'>
                      <span className='absolute left-0 top-0 text-4xl text-indigo-500 opacity-20'>
                        "
                      </span>
                      ...
                      <span className='absolute right-0 bottom-0 text-4xl text-indigo-500 opacity-20 rotate-180'>
                        "
                      </span>
                    </div>
                    <div className='flex justify-end gap-2 mt-4'>
                      <button
                        onClick={() =>
                          handleFavorite(
                            typeof quote === "string" ? quote : String(quote)
                          )
                        }
